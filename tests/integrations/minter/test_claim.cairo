...
from tests.integrations.minter.library import setup
...
func test_claim_nominal_case{
    ...
    %{
        ids.minter_contract_address = context.minter.contract_address
        ids.badge_contract_address = context.badge.contract_address
        ids.whitelisted_user = context.minter.whitelisted_user
        ids.sig0 = context.minter.sig0
        ids.sig1 = context.minter.sig1
        ids.badge_type = context.minter.badge_type
    %}

    // Claim the badge
    %{ stop_prank_callable = start_prank(context.minter.whitelisted_user, target_contract_address=context.minter.contract_address) %}
    ...
    ICarbonableMinter.claim(minter_contract_address, (sig0, sig1), badge_type);
    %{ stop_prank_callable() %}
...
func test_claim_revert_invalid_user{
    ...
    // Claim badge
    %{ stop_prank_callable = start_prank(ids.invalid_user, target_contract_address=context.minter.contract_address) %}
    ...
    %{ expect_revert("TRANSACTION_FAILED", "CarbonableMinter: invalid signature") %}
    ICarbonableMinter.claim(minter_contract_address, (sig0, sig1), badge_type);
